...
Function ChkIsNew1()
	if Topic_Status <> 3 then
		UnApprovedFound = "Y"
		ChkIsNew1 = getCurrentIcon(strIconFolderUnmoderated,"Post(s) Awaiting Approval","")
	else
		HeldFound = "Y"
		ChkIsNew1 = getCurrentIcon(strIconFolderHold,"Post is on Hold","")
	end if
End Function
...
Function ChkIsNew2(dt)
	...
	if dt > Session(strCookieURL & "last_here_date") then
		if Topic_Replies >= intHotTopicNum and lcase(strHotTopic) = "1" Then
			ChkIsNew2 = getCurrentIcon(strIconFolderNewHot,"Hot Topic","")
		else
			ChkIsNew2 = getCurrentIcon(strIconFolderNew,"New Topic","")
		end if
	elseif Topic_Replies >= intHotTopicNum and lcase(strHotTopic) = "1" Then
		ChkIsNew2 = getCurrentIcon(strIconFolderHot,"Hot Topic","")
	else
		ChkIsNew2 = getCurrentIcon(strIconFolder,"","")
	end if
End Function
%>
