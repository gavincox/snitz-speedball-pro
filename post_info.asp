	...
	if rs.BOF or rs.EOF or not(QuoteOk) or not (ChkQuoteOk(strPassword))then '##  Invalid Password
		...
		if strSignatures = "1" and strDSignatures <> "1" then
			if Request.Form("sig") = "yes" and GetSig(strDBNTUserName) <> " " then
				txtMessage = txtMessage & strLE & strLE & ChkString(GetSig(strDBNTUserName), "signature" )
			end if
		end if
...
if MethodType = "Reply" or MethodType = "ReplyQuote" or MethodType = "TopicQuote" then
	...
	if rs.BOF or rs.EOF or not(QuoteOk) or not(ChkQuoteOk(strPassword)) then '##  Invalid Password
		rs.close
		saMsg = "Invalid Password or User Name"
		Go_Result(err_Msg), 0
		Response.End
	else
...
sub Go_Result(str_err_Msg, boolOk)
	...
	if MethodType = "Topic" or _
		...
		MethodType = "EditTopic" then
			...
			Response.Write "&nbsp;<a href=""default.asp?CAT_ID=" & Cat_ID & """>" & ChkString(strCatTitle, "title") & "</a><br>" & strLE
			...
			Response.Write "&nbsp;<a href=""forum.asp?FORUM_ID=" & Forum_ID & """>" & ChkString(strForum_Title, "title") & "</a><br>" & strLE
	end if
	if MethodType = "Reply" or _
		MethodType = "ReplyQuote" or _
		MethodType = "TopicQuote" or _
		...
		MethodType = "EditTopic" then
	   		...
	   		Response.Write "&nbsp;<a href=""" & chkString(Request.Form("refer"),"refer") & """>" & ChkString(strTopicTitle,"title") & "</a>" & strLE
	end if
	...
	if boolOk = 1 then
		...
		Response.Write "</span></p>" & strLE & _
				"<meta http-equiv=""Refresh"" content=""2; URL=" & strReturnURL & """>" & strLE & _
				"<p class=""c""><span class=""dff dfs"">"
		select case MethodType
			case "Category"
		...
		Response.Write "</span></p>" & strLE & _
				"<p class=""c""><span class=""dff dfs""><a href=""" & strReturnURL & """>" & strReturnTxt & "</a></span></p>" & strLE
	else
		Response.Write "<p class=""c""><span class=""dff hfs hlfc"">There has been a problem!</span></p>" & strLE & _
				"<p class=""c""><span class=""dff dfs hlfc"">" & str_err_Msg & "</span></p>" & strLE & _
...
sub DoAutoMoveEmail(TopicNum)
	...
	If Not rs2.EOF Then
		...
		if lcase(strEmail) = "1" then
			...
			strMessage        = "Hello " & user_name & strLE & strLE
			strMessage        = strMessage & "Your posting on " & strForumTitle & "." & strLE
			strMessage        = strMessage & "Regarding the subject - " & Topic_Title & "." & strLE & strLE

			if not(chkForumAccess(ForumID,Usernum,false)) then
				strMessage = strMessage & "Has been removed from public display, If you have any questions regarding this, please contact the Administrator of the forum" & strLE
			else
				strMessage = strMessage & "Has been moved to a new forum, You can view it at " & strLE & Left(Request.Form("refer"), InstrRev(Request.Form("refer"), "/")) & "topic.asp?TOPIC_ID=" & TopicNum & strLE
			end if
%>
