	...
	if rsLoop.EOF or rsLoop.BOF then
		...
		for iSub = 0 to LoopCount
			...
			if chkForumAccess(ForumID, LoopMemberID, false) <> FALSE then
				...
				strMessage = "Hello " & LoopMemberName & strLE & strLE
				...
				strMessage = strMessage & "Regarding the subject - " & TopicName & "." & strLE & strLE
				strMessage = strMessage & "You can view the posting at " & strForumURL & "topic.asp?whichpage=-1&TOPIC_ID=" & TopicId & strLE
%>
...
Function ShowSubLink (SubOption, CatID, ForumID, TopicID, ShowText)
	Dim DefaultFont
	DefaultFont = "<span class=""dff dfs"">"
	' -- Declare variables...
	...
	if Instr(Request.ServerVariables("SCRIPT_NAME"),"post.asp") then
		...
		ShowSubLink = StandardLinkInfo & LinkLevel & LinkParam & "')"">" & getCurrentIcon(LinkIcon, LinkText,"class=""vam""") & "</a>"
		if ShowText <> "N" then
			ShowSubLink = ShowSubLink & "&nbsp;" & StandardLinkInfo & LinkLevel & LinkParam & "')"">" & DefaultFont & LinkText & "</span></a>"
		end if
	end if
end function
%>
